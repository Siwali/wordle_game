 ...
 */
function deriveLetterStatus(guesses) {
 ...
 */
function shouldUpgradeStatus(current, newStatus) {
    if (current === 'correct')
        return false; // Already highest
    if (current === 'present')
        return newStatus === 'correct';
    if (current === 'absent')
        return newStatus === 'correct' || newStatus === 'present';
    return true;
}
/**
 * Get status class name for a letter
 */
function getStatusClassName(letter, statusMap) {
 ...
 */
export function createKeyboard(guesses, onEvent) {
 ...
 */
function createKeyboardRow(keys, statusMap, onEvent) {
 ...
 */
function createKey(key, statusMap, onEvent) {
